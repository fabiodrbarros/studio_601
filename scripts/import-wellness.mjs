...
try{
 ...
 const path=join(directory,'before-wellness-'+new Date().toISOString().replace(/[:.]/g,'-')+'.sqlite');await backup(db,path);
 db.exec('BEGIN IMMEDIATE');
 try{
  const row=readStoredCatalog(),previous=row?JSON.parse(String(row.data)):structuredClone(empty);
  const {data,report}=importWellness(previous);schema.parse(data);
